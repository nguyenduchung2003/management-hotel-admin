...
export default function PropertyManagementView() {
    ...
    const handleCreateOrUpdate = (values: any) => {
        if (editingProperty) {
            ...
            message.success(
                `Đã cập nhật thông tin "${values.name}" thành công!`,
            )
        } else {
            // Create new
            const newProp: Property = {
                ...
                cancellationPolicy:
                    values.cancellationPolicy || "Miễn phí hủy trước 3 ngày.",
                status: values.status || "active",
                roomTypes: [],
            }
            setProperties([newProp, ...properties])
            message.success(
                `Đã thêm cơ sở lưu trú mới "${values.name}" thành công!`,
            )
        }
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_: any, record: Property) => (
                <Badge
                    count={`${record.roomTypes.length} loại phòng`}
                    showZero
                    color="blue"
                />
            ),
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-xs border border-slate-200/60 dark:border-slate-800">
                <div>
                    ...
                    >
                        Quản lý danh sách, địa chỉ, tiện ích và trạng thái hoạt
                        động của các cơ sở lưu trú.
                    </Text>
                </div>
                <Button
                    type="primary"
            ...
            <div className="flex flex-col gap-3">
                ...
                <Card className="border-none shadow-sm rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800">
                    <Row gutter={[16, 16]} items-center>
                        <Col xs={24} sm={10} md={10}>
                            <Input
                                prefix={
                                    <SearchOutlined className="text-gray-400 dark:text-slate-500" />
                                }
                        ...
                        <Col xs={12} sm={7} md={7}>
                            ...
                            >
                                <Option value="all">
                                    Tất cả loại hình (Hotel & Homestay)
                                </Option>
                ...
                <Drawer
                    title={
                        editingProperty
                            ? `Chỉnh sửa: ${editingProperty.name}`
                            : "Thêm mới Cơ sở lưu trú"
                    }
                    width={600}
                ...
                >
                    ...
                    >
                        ...
                        >
                            <Input.TextArea
                                rows={2}
                                placeholder="Nhập quy định hủy phòng..."
                            />
                        </Form.Item>
                        ...
                        <Form.Item name="description" label="Mô tả giới thiệu">
                            <Input.TextArea
                                rows={3}
                                placeholder="Mô tả nổi bật về không gian, vị trí..."
                            />
                        </Form.Item>

                        <Form.Item
                            name="amenityIds"
                            label="Các tiện ích cung cấp"
                        >
                    ...
                    <div className="space-y-4 py-2">
                        ...
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    <Tag color="blue">
                                        {detailModalProperty.type.toUpperCase()}
                                    </Tag>
                                    <Tag color="cyan">
                                        {detailModalProperty.city}
                                    </Tag>
                                </div>
                            </div>
                            <div className="text-right">
                                <Rate
                                    disabled
                                    defaultValue={detailModalProperty.rating}
                                    className="text-sm"
                                />
                                <div className="text-xs text-gray-400">
                                    {detailModalProperty.reviewCount} đánh giá
                                    từ khách
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="border-t border-gray-100 dark:border-gray-800 pt-3">
                            <Text strong className="block mb-2 text-sm">
                                🛏️ Các Loại Phòng Thuộc Cơ Sở (
                                {detailModalProperty.roomTypes.length}):
                            </Text>
                            {detailModalProperty.roomTypes.length > 0 ? (
                                        ...
                                        >
                                            ...
                                            <div className="text-right">
                                                <div className="font-bold text-blue-600">
                                                    ...
                                                    ).format(rt.pricePerNight)}
                                                    /đêm
                                                </div>
                                                <div className="text-gray-400">
                                                    Còn {rt.availableRooms}/
                                                    {rt.totalRooms} phòng
                                                </div>
                                            </div>
                                        </div>
                                ...
                                >
                                    Chưa có loại phòng nào được tạo cho cơ sở
                                    này.
                                </Text>
                            )}
                        </div>
