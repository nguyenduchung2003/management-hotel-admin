...
export default function ReviewManagementView() {
    ...
    const handleReplySubmit = (values: any) => {
        ...
        setReviews(updated)
        message.success(
            `Đã gửi phản hồi cho khách hàng "${selectedReview.guestName}"!`,
        )
    ...
    const filteredReviews = reviews.filter((r) => {
        const matchesProperty =
            propertyFilter === "all" || r.propertyId === propertyFilter
        const matchesRating =
            ratingFilter === "all" ||
    ...
    return (
        <div className="space-y-6">
            {/* Header */}
            <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-xs border border-slate-200/60 dark:border-slate-800">
                <Title
                    level={4}
                    style={{ margin: 0 }}
                ...
                <Text type="secondary" className="text-xs dark:text-slate-400">
                    Lắng nghe phản hồi từ khách hàng, phản hồi đánh giá và cải
            ...
            <div className="flex flex-col gap-3">
                ...
                <div className="space-y-4 flex flex-col gap-3">
                    {filteredReviews.map((item) => (
                        <Card
                            key={item.id}
                            className="border-none shadow-sm rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800"
                        >
                            ...
                            {item.reply && (
                                <div className="bg-blue-50 dark:bg-slate-800/80 p-3 rounded-lg border-l-4 border-blue-500 text-xs">
                                    <div className="font-bold text-blue-700 dark:text-blue-400 flex items-center gap-1 mb-1">
                                        <CheckCircleOutlined /> Phản hồi từ Ban
                                        Quản Lý:
                                    </div>
