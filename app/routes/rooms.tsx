...
const initialRoomTypes: RoomType[] = INITIAL_PROPERTIES.flatMap(
    (p) => p.roomTypes,
)
...
export default function RoomManagementView() {
    ...
    const filteredRoomTypes = roomTypes.filter((rt) => {
        const matchesProperty =
            selectedPropertyId === "all" || rt.propertyId === selectedPropertyId
        const matchesSearch =
    ...
    const handleSave = (values: any) => {
        const targetProp = INITIAL_PROPERTIES.find(
            ...
            render: (_: any, record: RoomType) => (
                <div>
                    ...
                    <div className="text-xs text-blue-600 font-medium flex items-center gap-1 mt-0.5">
                        <HomeOutlined />{" "}
                        {record.propertyName || "Cơ sở lưu trú"}
                    </div>
                </div>
            ),
        },
                    ...
                    <Badge
                        ...
                        text={
                            <span className="font-semibold text-xs">
                                {record.availableRooms} / {record.totalRooms}{" "}
                                phòng trống
                            </span>
                        }
                    />
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-xs border border-slate-200/60 dark:border-slate-800">
                <div>
                    ...
                    >
                        Quản lý cấu hình các loại phòng, thiết lập giá theo đêm,
                        sức chứa và số lượng phòng khả dụng.
                    </Text>
                </div>
                <Button
                    type="primary"
            ...
            <Modal
                title={
                    editingRoom
                        ? `Sửa loại phòng: ${editingRoom.name}`
                        : "Thêm Loại Phòng Mới"
                }
                open={isModalOpen}
            ...
            >
                ...
                >
                    ...
                    <Form.Item name="description" label="Mô tả chi tiết">
                        <Input.TextArea
                            rows={3}
                            placeholder="Đặc điểm không gian, nội thất phòng..."
                        />
                    </Form.Item>
                </Form>
            </Modal>
